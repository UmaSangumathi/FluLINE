...
# Read the summary info file:
  ...
  for x in SeqIO.parse(open(ref, "r"), "fasta"):
    ...
    # combine all seqments into one ref...
      ...
      for i in SeqIO.parse(open(cons_ref, "r"), "fasta"):  # Cahnged to input reference
        with open(Cons_ref_ful, "a") as f :   
	  print i
          f.write(">" + info_dict[ddd][1].replace(" ","") + "|" + i.id.split("-")[1] + "|" + "\r\n" + str(i.seq) + "\r\n")
  else:
    print "No Coverage :( "
  ...
  # run lofreq2....
    ...
    # C option : 100 is min depth required to call SNPs
    print bam_cmd
    os.system(bam_cmd)
    print bam_sort
    os.system(bam_sort)
